resource "azurerm_virtual_machine" "LzPcADC-F5VM02" {

name = "LzPcADC-F5VM02"
location = "canadaCentral"
resource_group_name = data.azurerm_resource_group.LzPc-Security_Core_External-rg.name
provider = azurerm.LzPc-PBMMCore
network_interface_ids = [azurerm_network_interface.LzPcADC-F5VM02-nic1.id, azurerm_network_interface.LzPcADC-F5VM02-nic2.id, azurerm_network_interface.LzPcADC-F5VM02-nic3.id]
primary_network_interface_id = azurerm_network_interface.LzPcADC-F5VM02-nic1.id
...
connection {
host     = azurerm_public_ip.LzPcADC-F5VM02-pip1.ip_address
type     = "ssh"
user     = "admf5"
password = "Canada1!"
}
...
resource "azurerm_virtual_machine_extension" "LzPcADC-F5VM02-run_startup_cmd" {
    ...
    resource_group_name = data.azurerm_resource_group.LzPc-Security_Core_External-rg.name
    provider = azurerm.LzPc-PBMMCore
    virtual_machine_name = azurerm_virtual_machine.LzPcADC-F5VM02.name
    ...
    settings = <<SETTINGS
        {
            "commandToExecute": "sleep 180; /bin/su -c 'bash /var/lib/waagent/CustomData 2' - admf5"
        }
    SETTINGS
...
output "LzPcADC-F5VM02-pip1" { value = azurerm_public_ip.LzPcADC-F5VM02-pip1.ip_address}